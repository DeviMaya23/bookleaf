...
describe('queryKeyFor', () => {
  it('includes a sort key for an explicit sort in the All view', () => {
    expect(queryKeyFor({ type: 'all' }, '', 'title', 'asc', EMPTY_FILTER, EMPTY_FILTER, EMPTY_FILTER))
      .toEqual(['images', 'all', '', ['title', 'asc'], EMPTY_FILTER, EMPTY_FILTER, EMPTY_FILTER])
  })

  it('omits the sort key when sortBy is manual', () => {
    expect(queryKeyFor({ type: 'unsorted' }, '', 'manual', undefined, EMPTY_FILTER, EMPTY_FILTER, EMPTY_FILTER))
      .toEqual(['images', 'unsorted', '', undefined, EMPTY_FILTER, EMPTY_FILTER])
  })

  it('keys the trash view without filters', () => {
    expect(queryKeyFor({ type: 'trash' }, 'foo', 'manual', undefined, EMPTY_FILTER, EMPTY_FILTER, EMPTY_FILTER))
      .toEqual(['images', 'trash', 'foo', undefined])
  })

  it('keys the folder view by folder id and sort, without search/filters', () => {
    expect(queryKeyFor({ type: 'folder', id: 'folder-1' }, 'foo', 'title', 'desc', ['tag-1'], EMPTY_FILTER, EMPTY_FILTER))
...
describe('fetcherFor', () => {
  ...
  it('calls getAllImages with sort, direction, and filters for the All view', async () => {
    ...
    const fetcher = fetcherFor({ type: 'all' }, getToken, 'foo', 'created_at', 'asc', ['tag-1'], ['image/jpeg'], ['folder-1'])
  ...
  it('calls getImages with no folder ids for the Unsorted view', async () => {
    ...
    const fetcher = fetcherFor({ type: 'unsorted' }, getToken, 'foo', 'manual', undefined, ['tag-1'], EMPTY_FILTER, ['folder-1'])
  ...
  it('calls getTrashedImages with only the search term for the Trash view', async () => {
    ...
    const fetcher = fetcherFor({ type: 'trash' }, getToken, 'foo', 'title', 'asc', EMPTY_FILTER, EMPTY_FILTER, EMPTY_FILTER)
    await fetcher({ pageParam: undefined })
  ...
  it('calls getFolderImages with the folder id, sort, and direction for the Folder view', async () => {
    ...
    const fetcher = fetcherFor({ type: 'folder', id: 'folder-1' }, getToken, '', 'title', 'desc', EMPTY_FILTER, EMPTY_FILTER, EMPTY_FILTER)
    await fetcher({ pageParam: undefined })
  ...
  return renderHook(
    ({ searchTerm }: { searchTerm: string }) =>
      useGalleryImages(
        ...
        'manual',
        undefined,
        props?.filterTagIds ?? EMPTY_FILTER,
        props?.filterMimeTypes ?? EMPTY_FILTER,
        props?.filterFolderIds ?? EMPTY_FILTER,
      ),

...
The content script SHALL render a transient drop zone, anchored at the pointer's `dragstart` client coordinates, if and only if `dragstart` resolved a non-null `srcUrl`. The drop zone SHALL be removed on `dragend`, regardless of whether a `drop` occurred. As a safety net for sites whose own drag-and-drop UI cancels native dragging (e.g. by calling `preventDefault()` on `dragstart`, which prevents `dragend` from ever firing), the content script SHALL also remove the drop zone on `pointerup` and `mouseup`.

#### Scenario: Drop zone appears for a resolvable image drag

- **WHEN** `dragstart` resolves a non-null `srcUrl`

...
7. Execute the 4-step upload sequence:
   a. `POST /images` → receives `{ upload_url, thumbnail_upload_url, id }`. If a default folder is configured in `browser.storage.local`, its `id` SHALL be sent as `folder_id` in the request body; otherwise `folder_id` SHALL be omitted.
...
8. Send a success toast to the active tab with title "Saved to Bookleaf." and body:
   - `"Added to [folder name]."` when a default folder is configured
   - `"Added to Unsorted."` when no default folder is configured

The image title SHALL be set per step 5 above (the current tab's title, `tab.title`, for saves with no matching Twitter permalink and no resolved Imgur/Instagram/Facebook text). The `source_url` sent to the backend SHALL be the ClearURLs-cleaned URL produced by `cleanUrl()` per step 4 above. If a default folder is configured in `browser.storage.local`, its `id` SHALL be sent as `folder_id` in the `POST /images` request body; otherwise `folder_id` SHALL be omitted. The `tabId` from the context menu event SHALL be threaded into the save handler and used for all `browser.tabs.sendMessage` calls.
...
- **WHEN** the user clicks "Save to Bookleaf" while authenticated and the image is fetchable and no default folder is configured
...
#### Scenario: Successful save with default folder uses folder_id and shows folder name in toast

- **WHEN** the user clicks "Save to Bookleaf" while authenticated, the image is fetchable, and a default folder `{ id: "f1", name: "Inspo" }` is configured
- **THEN** `POST /images` is sent with `folder_id: "f1"` in the request body
- **AND** an in-page toast with title "Saved to Bookleaf." and body "Added to Inspo." is shown in the active tab

...
The system SHALL expose a `GET /me` endpoint that returns the authenticated user's Kinde ID, `vision_enabled` flag, `folder_icons_enabled` flag, and `ai_categorisation_enabled` flag. The endpoint SHALL be in the protected route group and require a valid JWT.

Response body (200):
```json
{ "id": "kp_abc123", "vision_enabled": false, "folder_icons_enabled": true, "ai_categorisation_enabled": false }
```
...
- `ai_categorisation_enabled` — boolean from the `users` table; read-only, not user-configurable

#### Scenario: Authenticated user retrieves their profile

- **WHEN** an authenticated request is made to `GET /me`
- **THEN** the response is `200 OK`
- **AND** the body contains the user's Kinde ID, `vision_enabled`, `folder_icons_enabled`, and `ai_categorisation_enabled` values
...
```json
{ "id": "kp_abc123", "vision_enabled": true, "folder_icons_enabled": false, "ai_categorisation_enabled": false }
```

- `vision_enabled` and `folder_icons_enabled` are the only writable fields. The request body SHALL NOT permit updating any other user column (`id`, `ai_categorisation_enabled`, `pending_kinde_deletion`, timestamps).
- `ai_categorisation_enabled` is included in the response body but is read-only and SHALL NOT be accepted as a PATCH input.
...
- **THEN** the response is `200 OK`
- **AND** the body is `{ "id": "<user id>", "vision_enabled": true, "folder_icons_enabled": <unchanged>, "ai_categorisation_enabled": <unchanged> }`
...
- **THEN** the response is `200 OK`
- **AND** the body is `{ "id": "<user id>", "vision_enabled": false, "folder_icons_enabled": <unchanged>, "ai_categorisation_enabled": <unchanged> }`
...
- **THEN** the response is `200 OK`
- **AND** the body is `{ "id": "<user id>", "vision_enabled": <unchanged>, "folder_icons_enabled": false, "ai_categorisation_enabled": <unchanged> }`

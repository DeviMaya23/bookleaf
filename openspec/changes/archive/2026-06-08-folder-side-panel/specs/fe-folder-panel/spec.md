...
- Both fields are pre-populated with the folder's current values and are independently auto-saved on blur via `PUT /folders/:id`, mirroring the image title/notes pattern in `RightPanel`
...
The system SHALL display the folder `name` as an editable input. Changes SHALL be auto-saved via `PUT /folders/:id` when the field loses focus, but only if the value has changed and is non-empty. The title SHALL NOT be saved as an empty string; if the user clears the field and blurs, the input SHALL revert to the previous value without making an update call.
...
- **AND** no `PUT /folders/:id` call is made

---

### Requirement: Folder description is editable with auto-save on blur

The system SHALL display the folder `description` as an editable notes field, pre-populated with the current value (empty if `null`). Changes SHALL be auto-saved via `PUT /folders/:id` when the field loses focus, but only if the value has changed. An empty description SHALL be saved as `null`, clearing it.
...
- **THEN** the app calls `PUT /folders/<id>` with the updated `description`
- **AND** a success toast is shown

#### Scenario: Description can be cleared

- **WHEN** the user clears a previously non-empty description and removes focus
- **THEN** the app calls `PUT /folders/<id>` with `description` set to `null`
- **AND** a success toast is shown

...
On a valid file drop, the system SHALL run the 3-step upload sequence (initiateUpload → putToR2 → completeUpload) using the `folderId` derived from the current route (`/folders/:folderId` or null for root/unsorted). No modal SHALL be shown. `description` and `source_url` SHALL NOT be set (they can be edited in the right panel after upload). A loading state SHALL be indicated by the overlay while the upload is in progress.
...
- **AND** `POST /images/:id/complete` is called

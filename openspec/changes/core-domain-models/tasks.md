...
- [ ] 2.1 Write `internal/domain/user.go` — `User` struct with `ID` (Clerk string ID, `TEXT` PK; no UUID field)
...
- [ ] 3.2 Write `migration/000001_create_users.up.sql` — `users` table (`id TEXT PRIMARY KEY` using Clerk's user ID, timestamps, `deleted_at`)
...
- [ ] 3.6 Write `migration/000003_create_images.up.sql` — `images` table with FK to users and nullable FK to folders, `ai_labels` as JSONB

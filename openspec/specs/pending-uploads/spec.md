...
Defines the `PendingUpload` domain struct, the `pending_uploads` database table, and the `PendingUploadRepository` interface. A `PendingUpload` record tracks an upload-in-progress and is atomically committed to the `images` table (and deleted) by `CompleteUpload`.

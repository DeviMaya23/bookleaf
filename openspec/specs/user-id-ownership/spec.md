...
Migrate the `users` table primary key from Kinde-issued subject strings to app-generated UUIDs, establishing a clean separation between Bookleaf's internal identity space and the IdP's identity space. The Kinde subject is retained as a non-FK lookup column (`idp_subject`).

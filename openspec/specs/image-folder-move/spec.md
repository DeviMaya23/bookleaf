...
Defines the `POST /images/:id/move-folder` endpoint and its supporting usecase and repository layers for atomically moving an image between folders (or to/from the unsorted state).
...
- If `to_folder_id` is non-null: a row `(imageID, to_folder_id)` SHALL be inserted with a fracdex position appended to the end of that folder. If the row already exists, the existing row SHALL be left unchanged — position is preserved.
...
2. If `toFolderID` is non-nil: if a row for `(imageID, toFolderID)` already exists, leave it unchanged (position preserved). If no row exists, compute fracdex position via `fracdex.KeyBetween(maxPosition, "")` where `maxPosition` is the current max position in `toFolderID` (empty string if folder has no images), and insert the new row.

#### Scenario: Move executes both delete and insert atomically

- **WHEN** `MoveImageFolder` is called with non-nil `fromFolderID` and non-nil `toFolderID` and the image is not already in `toFolderID`
...
#### Scenario: Move to destination where image already exists preserves position

- **WHEN** `MoveImageFolder` is called with non-nil `toFolderID` and a row `(imageID, toFolderID)` already exists in `image_folders`
- **THEN** the existing `(imageID, toFolderID)` row is not modified
- **AND** its `position` value is unchanged

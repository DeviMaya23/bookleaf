...
Defines the `GET /images/in-folder/:id` endpoint and its supporting usecase and repository methods. This endpoint returns the full, ordered set of images belonging to a single folder — the dedicated "board/reorder view" query that was previously served via `GET /images?folder_id=...`'s folder-view branch (now removed from `image-endpoints`).

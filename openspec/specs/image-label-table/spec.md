# Spec: image-label-table

## Purpose

Normalised storage for AI-generated image labels. Introduces the `image_labels` table and the `ImageLabel` domain type, replacing the flat JSONB `ai_labels` column as the primary store for per-label rows. Supports efficient label queries (score filtering, frequency aggregation) without full JSONB parsing in Go.

...
This spec covers duplicate image detection: a repository method for finding perceptually similar images via Hamming distance, the `POST /images/:id/complete` endpoint accepting a client-computed pHash and returning duplicate matches, and a periodic background job that backfills pHash values for images uploaded before client-side hashing was introduced.

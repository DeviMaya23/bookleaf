...
3. For each image, generate `ThumbnailURL` (nil if `ThumbnailPath` is nil) and `FullResURL` via `StorageService.GeneratePresignedGetURL` with the existing `presignedGetTTL`.
...
- **AND** each image has a non-empty `FullResURL`
...
- **AND** the body contains `folder.name`, `folder.notes`, and an `images` array with `title`, `thumbnail_url`, and `full_res_url` per image
...
#### Scenario: Public route does not require authentication

- **WHEN** the server starts
- **THEN** `GET /share/:token` does not require a Kinde Bearer token
...
The system SHALL have unit tests for `ShareHandler` using value-return spy `ShareUsecase`, covering:

...
After labels are retrieved from Vision API, the system SHALL derive a single `FolderSuggestion` from the highest-scoring label by calling `FolderRepository.FindByName` with the label description and the image's `UserID`.

The resolved suggestion SHALL be:
- `FolderID`: the matched folder's UUID if found, otherwise `nil`
- `FolderName`: the label description (as returned by Vision, unmodified)
- `IsNew`: `true` if no existing folder matched, `false` otherwise

#### Scenario: Matching folder exists

- **WHEN** the top label is `"Nature"` and the user has a folder named `"nature"`
- **THEN** `FolderSuggestion.FolderID` is that folder's UUID
- **AND** `FolderSuggestion.FolderName` is `"Nature"`
- **AND** `FolderSuggestion.IsNew` is `false`

#### Scenario: No matching folder

- **WHEN** the top label is `"Architecture"` and the user has no folder by that name
- **THEN** `FolderSuggestion.FolderID` is `nil`
- **AND** `FolderSuggestion.FolderName` is `"Architecture"`
- **AND** `FolderSuggestion.IsNew` is `true`

#### Scenario: Vision returns no labels

- **WHEN** Vision API returns an empty label list
- **THEN** `FolderSuggestion` is `nil`

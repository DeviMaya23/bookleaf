...
Provide a server-side maintenance mode that gates protected routes behind a `503 Service Unavailable` response when enabled, with a configurable bypass mechanism for operators and a header that frontend clients can use to detect maintenance state.
